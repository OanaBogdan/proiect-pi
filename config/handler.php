...
if (isset($_POST['add_review'])){
  ...
  //if comment empty let it be null
  if(empty($comment)){
    $sql = "INSERT INTO ratings (value, product_id, user_id) VALUES ('$rating', '$product_id', '$user_id')";
  }else{
    $sql = "INSERT INTO ratings (value, content, product_id, user_id) VALUES ('$rating', '$comment', '$product_id', '$user_id')";
  }
  $result = mysqli_query($conn, $sql);
  if($result){
    header("Location: ../product.php?id=$product_id&success=Review-ul a fost adăugat cu succes");
    exit();
  }else{
    header("Location: ../product.php?id=$product_id&error=Review-ul nu a putut fi adăugat");
    exit();
  }
}
